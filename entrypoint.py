...
def record_engine_temperature():
    ...
    print(engine_temperature_values)
...
def collect_engine_temperature():
    database = redis.Redis(host="redis", port=6379, db=0, decode_responses=True)
    engine_temperature_values = database.lrange(DATA_KEY, 0, -1)
    logger.info(f"List of temperature values pulled from redis:{engine_temperature_values}")
    
    engine_temperature_values2 = [float(i) for i in engine_temperature_values]
    
    average_temperature = sum(engine_temperature_values2)/len(engine_temperature_values2)
    logger.info(f"The average temperature of the engine is: {average_temperature}")
    logger.info(f"The current temperature of the engine is: {engine_temperature_values[0]}")
    
    return {"current_temperature": engine_temperature_values[0], "average_temperature": average_temperature}, 200
